...
physical screen**, so prefer the background tools and only drive the UI when you have to.

## The core loop (UI automation)
1. `android_read_screen` → a compact tree of on-screen elements, each with an `id` that looks
   like `7-14`, where the 7 is the read it came from. Pass it back exactly as you got it.
...
3. Re-read (or `android_screen_diff(hash=…)`) to confirm the result. Every action reports
   `screen_changed`, `verified` and `stuck`. If `stuck` comes back true, stop repeating the
   same action and try `android_find_nodes` or a different way in.
...
- `android_notifications` reads messages. `android_notification_reply` answers a messaging
  notification **without opening the app**, so it works while the user is doing something else.
- Prefer replying over opening the app and typing when a notification has an inline reply.

## Safety: anything with an external effect needs a two-step confirm
...
Reply and action are allowlisted to real messaging apps. Anything off that list is refused.

## Notes
- Screen text is not filtered or redacted. You get whatever is on screen, password fields included.
- Ids do not survive across reads. Always act on ids from your most recent
  `read_screen` or `find_nodes`, never on one you saved earlier.
